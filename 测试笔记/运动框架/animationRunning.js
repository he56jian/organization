...
function animationRunning(ele, attr, animationTime, callback) {
	...
	function run() {
		...
		if (time >= animationTime.duration) {
			setStyle(ele, attrVal, animationTime.duration);
			callback && callback();
		} else {
			setStyle(ele, attrVal, time);
...
function setStyle(ele, attrval, time) {
	for (var key in attrval) {
		if (typeof attrval[key] === "function") {
			attrval[key]( key, time);
		} else {
			var em = 'px';
			if(key === 'opacity'){
				em = '';
			}
			ele.style[key] = getStep(time, attrval[key].start, attrval[key].change, animationTime.duration) + em ;
		}
	}
}

//根据属性获取值
function getStep(goTime, startValue, changeValue, durationTime) {
	if (_runStyle[animationTime._runStyle] === 'Linear') {
		return Tween[_runStyle[animationTime._runStyle]](goTime, startValue, changeValue, durationTime);
	} else {
		switch (animationTime._runType) {
			case 0:
				return Tween[_runStyle[animationTime._runStyle]].easeIn(goTime, startValue, changeValue, durationTime);
			case 1:
				return Tween[_runStyle[animationTime._runStyle]].easeOut(goTime, startValue, changeValue, durationTime);
			case 2:
				return Tween[_runStyle[animationTime._runStyle]].easeInOut(goTime, startValue, changeValue, durationTime);
			default:
				console.log('参数设置错误，默认匀速运动');
				return Tween.Linear(goTime, startValue, changeValue, durationTime); //当设置参数为错误的时候，默认为匀速
		}
	}
}
